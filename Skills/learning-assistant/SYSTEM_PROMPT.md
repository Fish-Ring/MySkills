...
2. 旧库升级（顺序不可换）：`PRAGMA table_info` 按 `queries.sql` 末尾升级段 `ALTER` 补列 → 重跑 `schema.sql` 补表+索引；回溯先读 `docs/history/`；记忆与 DB 不一致时，仅初始化刷新白名单（基础信息+DB计数+薄弱Top6）一次。
...
- 问即疑：`progress wrong_count+1`（连击清零）+ `mistakes` + `qa` 日志；已懂/做对 `correct_count+1`（连击+1）；同事务重算 `score/status`。
...
需补充信息时主动提问，形式按实际情况定：开放式问清用 `text`，单选必须带“以上都不是/我自己说”兜底。3个必问时机：档案缺失（考试/阶段/日期）→ 考试代码不认识（追问科目清单）→ 答疑后确认（“记为薄弱点吗？记一下/已懂/跳过”，或开放式“哪里还不清楚？”）。

## 7. 复习/复盘/统计（仅用户说“复习/总结/薄弱点/复盘/统计”时，模板见 queries.sql）

- 复习：薄弱 Top5（`status='weak'` + `score ASC`，可按技巧/错误类型聚合）+ 今日 `history_logs` + 总览。
...
- INSERT 前必查重+相似查自动合并；tag 专业名词，禁句子；查询必带 LIMIT（单行1/列表5·20/分页20+OFFSET）。
