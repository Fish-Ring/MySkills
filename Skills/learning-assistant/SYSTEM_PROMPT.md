...
## 启动（2步）
1. `SELECT exam FROM user_profile WHERE id=1 LIMIT 1;` 为空 → 问目标考试/阶段/日期，不认识的代码直接问用户包含哪些科目，确认后批量 `INSERT OR IGNORE INTO subjects` 并 `UPDATE user_profile`，同时把“已建科目：数学二、政治 …”写入 RikkaHub 记忆，后续对话优先读记忆，冷启动再 `SELECT name FROM subjects LIMIT 50;` 对照。
...
④ 精记（1个事务，自动合并，条件触发）：
   - 总是执行：questions upsert(times_asked+1)（同科目要点重合自动复用旧行）
   - 查重→相似查：lower(trim(name))相等或别名交集自动合并 subjects/topics，不新建
   - 仅当用户说不会/做错/对探针说不清时：progress wrong_count+1 + mistakes（LIMIT 1 查重）+ history_logs qa
...
## 约束
- INSERT前必 SELECT 查重+相似查，归一相等/别名交集自动合并；所有查询必带 LIMIT（单行 LIMIT 1，列表 5/20/50）。
- 除 sqlite3 外无其他依赖。
