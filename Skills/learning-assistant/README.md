# Learning Assistant (通用学习助手) v1.6.0
...
# 回溯演变：cat <技能目录>/docs/history/v1.6.0.md（技能目录自带）
```
...
| 记录 | 问即疑（以掌握确认为准）：只 INSERT 原子事实，重算与日志触发器自动做，禁手写；写库后必回“已入库”摘要 |
...
| 公式 | 表格单元格内禁 `$` `\` 命令（改 Unicode：⁻¹₀₁→×÷≠≤≥∈√∞∑Δθλπ°），正文 LaTeX 可用 |
