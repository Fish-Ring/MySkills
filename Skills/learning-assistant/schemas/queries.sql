-- 通用学习助手 v1.4.0 - 常用 SQL 模板（精简版：2次命中封顶）
-- 用法：sqlite3 -json <技能目录>/learner.db "<语句>"
-- 约定：' 拼入前写成 ''；先查重后写入

-- ============ 查重（INSERT 前必跑） ============
SELECT id FROM subjects WHERE name='科目名';
SELECT id FROM topics WHERE subject_id=科目ID AND name='知识点';
SELECT id, times_asked FROM questions WHERE question='问题原文';
SELECT id FROM mistakes WHERE topic_id=知识点ID AND question='题目' AND COALESCE(wrong_answer,'')=COALESCE('错答','');
...
UPDATE topics SET keywords=CASE WHEN keywords='' OR keywords IS NULL THEN '别名' ELSE '别名,'||keywords END
  WHERE id=知识点ID AND (','||COALESCE(keywords,'')||',') NOT LIKE '%,别名,%';
...
-- ============ 核心检索：预答→精查薄弱点（1条批量，带分科，问答时唯一必查） ============
...
WHERE t.subject_id=(SELECT id FROM subjects WHERE name='判定的科目')
  AND t.name IN ('要点1','要点2');  -- 要点来自预答草拟的2-3个核心知识点名
