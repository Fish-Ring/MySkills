# 练习模块（v1.6.0，冷酷阅卷官，仅用户说练题/出题时用，不主动出题）
...
```
无薄弱点按大纲出题；做错走 SYSTEM_PROMPT §5 步骤4同款事务（mistakes + 判定类型入 misconceptions + progress wrong_count+1 连击清零，重算与 mistake 日志触发器自动做禁手写）；做对 correct_count+1 连击+1 + 手写 exercise 日志（queries.sql 日志段）。
